"""
    ...
    This example demonstrates and evaluates the operation-splitting memory
...
def load_and_export_model(args):
    ...
    if attempt_seq:
      ...
      seq_opt = tfm_seq_opt.SeqAllocateGraph(load_and_sequence_graph(flatbuffer))
    ...
    mem_opt = tfm_heap_smart_order.HeapSmartOrder(load_and_sequence_graph(flatbuffer),
                                                  {'order': 'forwards'})
...
def main():
  ...
  parser.add_argument('-pg', '--print_graph',
                      action="store_true",
                      help='Generates am SVG vector graphic showing the '
                           'original input models flow graph.')
