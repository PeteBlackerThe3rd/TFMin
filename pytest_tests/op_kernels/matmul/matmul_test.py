...
# TODO need to complete modifting this conv2d test to test the matmul operation
def not_a_test_conv2d():

  test_list = get_test_list(ranges)
  print("Found a test list of %d tests" % len(test_list))
  for test in test_list:
    print("------ Starting test [%s] -------" % str(test))

    build_model(test)
    exit_code = subprocess.call(["./test_binary"])
